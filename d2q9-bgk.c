...
float timestep_new(const t_param params, t_speed* cells, t_speed* tmp_cells, int* obstacles);
int timestep_old(const t_param params, t_speed* cells, t_speed* tmp_cells, int* obstacles);
float timestep_new2(const t_param params, t_speed* cells, t_speed* tmp_cells, int* obstacles);


int accelerate_flow(const t_param params, t_speed* restrict cells, int* obstacles);
int propagate(const t_param params, t_speed* cells, t_speed* tmp_cells);
int propagate_new(const t_param params, t_speed* cells, t_speed* tmp_cells, int ii, int jj);
int propagate_fast(const t_param params, t_speed* cells, t_speed* tmp_cells);


int rebound(const t_param params, t_speed* restrict cells, t_speed* restrict tmp_cells, int* obstacles);
int rebound_new(const t_param params, t_speed* restrict cells, t_speed* restrict tmp_cells, int* obstacles, int ii, int jj);
int rebound_old(const t_param params, t_speed* cells, t_speed* tmp_cells, int* obstacles);

int collision(const t_param params, t_speed* restrict cells, t_speed* restrict tmp_cells, int* obstacles);
int collision_new(const t_param params, t_speed* cells, t_speed* tmp_cells, int* obstacles, int ii, int jj);
int collision_old(const t_param params, t_speed* cells, t_speed* tmp_cells, int* obstacles);
float collision_and_vel(const t_param params, t_speed* restrict cells, t_speed* restrict tmp_cells, int* obstacles);
...
inline void swap(t_speed** cells_ptr, t_speed **tmp_cells_ptr) {
...
int main(int argc, char* argv[])
{
  ...
  {
    av_vels[tt] = timestep_new2(params, cells, tmp_cells, obstacles);
    //timestep_new2(params, cells, tmp_cells, obstacles);
...
float timestep_new2(const t_param params, t_speed* cells, t_speed* tmp_cells, int* obstacles) {
  ...
  if (obstacles[jj*params.nx + ii]) {
    ...
    tmp_cells[ii + jj*params.nx].speeds[1] = cells[x_w + jj*params.nx].speeds[1]
                                              + params.omega
                                              * (d_equ[1] - cells[x_w + jj*params.nx].speeds[1]);

    tmp_cells[ii + jj*params.nx].speeds[2] = cells[ii + y_s*params.nx].speeds[2]
                                              + params.omega
                                              * (d_equ[2] - cells[ii + y_s*params.nx].speeds[2]);                                          
    
    tmp_cells[ii + jj*params.nx].speeds[3] = cells[x_e + jj*params.nx].speeds[3]
                                              + params.omega
                                              * (d_equ[3] - cells[x_e + jj*params.nx].speeds[3]);

    tmp_cells[ii + jj*params.nx].speeds[4] = cells[ii + y_n*params.nx].speeds[4]
                                              + params.omega
                                              * (d_equ[4] - cells[ii + y_n*params.nx].speeds[4]);
    
    tmp_cells[ii + jj*params.nx].speeds[5] = cells[x_w + y_s*params.nx].speeds[5]
                                              + params.omega
                                              * (d_equ[5] - cells[x_w + y_s*params.nx].speeds[5]);
    
    tmp_cells[ii + jj*params.nx].speeds[6] = cells[x_e + y_s*params.nx].speeds[6]
                                              + params.omega
                                              * (d_equ[6] - cells[x_e + y_s*params.nx].speeds[6]);
    
    tmp_cells[ii + jj*params.nx].speeds[7] = cells[x_e + y_n*params.nx].speeds[7]
                                              + params.omega
                                              * (d_equ[7] - cells[x_e + y_n*params.nx].speeds[7]);
    
    tmp_cells[ii + jj*params.nx].speeds[8] = cells[x_w + y_n*params.nx].speeds[8]
                                              + params.omega
                                              * (d_equ[8] - cells[x_w + y_n*params.nx].speeds[8]);
...
float timestep(const t_param params, t_speed* cells, t_speed* tmp_cells, int* obstacles)
{
  ...
  float vel = collision_and_vel(params, cells, tmp_cells, obstacles);
  return vel;
}
...
float timestep_new(const t_param params, t_speed* cells, t_speed* tmp_cells, int* obstacles) {
  ...
  rebound(params, cells, tmp_cells, obstacles);
  return collision_and_vel(params, cells, tmp_cells, obstacles);
}
...
int rebound_new(const t_param params, t_speed* cells, t_speed* tmp_cells, int* obstacles, int ii, int jj) {
  ...
  for (int jj = 0; jj < params.ny; jj++)
  {
    for (int ii = 0; ii < params.nx; ii++)
    {
      ...
      if (obstacles[jj*params.nx + ii])
      {
        /* called after propagate, so taking values from scratch space
        ** mirroring, and writing into main grid */
        float temp = tmp_cells[ii + jj*params.nx].speeds[1];
...
int collision_new(const t_param params, t_speed* restrict cells, t_speed* restrict tmp_cells, int* obstacles, int ii, int jj) {
  const float c_sq = 1.f / 3.f; /* square of speed of sound */
  const float w0 = 4.f / 9.f;  /* weighting factor */
  const float w1 = 1.f / 9.f;  /* weighting factor */
  const float w2 = 1.f / 36.f; /* weighting factor */
